...
export default function EditArea({setName, setIssuer, setWorkload, setDescription, setFinalDate, setImage, setPage, handleSubmit, handleDelete, item, openedItem, editMode}) {
  ...
  if (openedItem === item.id && editMode === true) {
    editArea = (
      <div id={item.id}>
        <input className="inputs" placeholder="Nome do aplicação" onChange={e => setName(e.currentTarget.value)} value={item.name}></input>
        <input className="inputs" placeholder="Emissor" onChange={(e => setIssuer(e.currentTarget.value))} value={item.issuer}></input>
        <input className="inputs" placeholder="Carga horária" onChange={(e => setWorkload(e.currentTarget.value))} value={item.workload}></input>
        ...
        <input className="inputs" placeholder="Data de término" onChange={e => setFinalDate(e.currentTarget.value)} value={item.finalDate}></input>
        <InputImageToDB
        image={item.image}
